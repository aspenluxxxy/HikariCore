...
void appendToAnnotations(Module& M, ConstantStruct* Data) {
	...
	if (AnnotationGV == nullptr) {
		...
		GlobalVariable* newGV =
			new GlobalVariable(M, CA->getType(), false, GlobalValue::AppendingLinkage, CA, "llvm.global.annotations");
		newGV->setSection("llvm.metadata");
		return;
	} else {
		StructType* ST = StructType::get(M.getContext(), {Int8PtrTy, Int8PtrTy, Int8PtrTy, Int32Ty});
		vector<Constant*> exists;
		for (unsigned i = 0; i < cast<ArrayType>(AnnotationGV->getInitializer()->getType())->getNumElements(); i++) {
		...
		ConstantArray* CA = cast<ConstantArray>(ConstantArray::get(AT, ArrayRef<Constant*>(exists)));
		GlobalVariable* newGV =
			new GlobalVariable(M, CA->getType(), false, GlobalValue::AppendingLinkage, CA, "llvm.global.annotations");
		newGV->setSection("llvm.metadata");
		return;
	...
	if (glob != nullptr && glob->hasInitializer()) {
		...
		for (Value* op : CDA->operands()) {
		  ...
		  We only care about Value and Annotation Here
			*/
			GlobalValue* Value = cast<GlobalValue>(anStruct->getOperand(0)->getOperand(0));
			GlobalVariable* Annotation = cast<GlobalVariable>(anStruct->getOperand(1)->getOperand(0));
			if (Annotation->hasInitializer()) {
				VAMap[Value] = cast<ConstantDataSequential>(Annotation->getInitializer())->getAsCString();
			}
		}
	}
	return VAMap;
...
bool toObfuscate(bool flag, Function* f, std::string attribute) {
	...
	// "nofla"
	if (readAnnotate(f).find(attrNo) != std::string::npos || readFlag(f, attrNo)) {
		return false;
	}
