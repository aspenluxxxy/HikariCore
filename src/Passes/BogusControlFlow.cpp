...
static cl::opt<int> ObfTimes("bcf_loop", cl::desc("Choose how many time the -bcf pass loop on a function"),
...
static Instruction::BinaryOps ops[] = {Instruction::Add, Instruction::Sub, Instruction::And, Instruction::Or,
									   Instruction::Xor};
static CmpInst::Predicate preds[] = {CmpInst::ICMP_EQ,	CmpInst::ICMP_NE,  CmpInst::ICMP_UGT,
...
static void RemoveDeadConstant(Constant* C) {
	...
	if (GlobalVariable* GV = dyn_cast<GlobalVariable>(C)) {
		...
		if (/* isa<CompositeType>(C->getType()) */ isa<StructType>(C->getType()) || isa<ArrayType>(C->getType()) ||
...
struct BogusControlFlow : public FunctionPass {
	...
	void bogus(Function& F) {
		...
		if (ObfProbRate < 0 || ObfProbRate > 100) {
			DEBUG_WITH_TYPE("opt", errs() << "bcf: Incorrect value,"
										  << " probability rate set to default value: " << defaultObfRate << " \n");
			ObfProbRate = defaultObfRate;
		}
		DEBUG_WITH_TYPE("opt", errs() << "bcf: How many times: " << ObfTimes << "\n");
		if (ObfTimes <= 0) {
			DEBUG_WITH_TYPE("opt", errs() << "bcf: Incorrect value,"
										  << " must be greater than 1. Set to default: " << defaultObfTime << " \n");
	...
	virtual void addBogusFlow(BasicBlock* basicBlock, Function& F) {
		...
		ICmpInst* condition2 = new ICmpInst(*originalBB, CmpInst::ICMP_EQ, LHS, RHS, "BCFPlaceHolderPred");
	...
	virtual BasicBlock* createAlteredBasicBlock(BasicBlock* basicBlock, const Twine& Name = "gen", Function* F = 0) {
					...
					for (int random = (int)llvm::cryptoutils->get_range(10); random < 10; ++random) {
						switch (llvm::cryptoutils->get_range(4)) { // to improve
							...
							case 1:
								...
								break;
							case 2:
								op1 = BinaryOperator::Create(Instruction::Sub, i->getOperand(0), i->getOperand(1), *var,
															 &*i);
								op = BinaryOperator::Create(Instruction::Mul, op1, i->getOperand(1), "gen", &*i);
								break;
							case 3:
								op = BinaryOperator::Create(Instruction::Shl, i->getOperand(0), i->getOperand(1), *var,
															&*i);
								break;
						}
					}
					...
					for (int random = (int)llvm::cryptoutils->get_range(10); random < 10; ++random) {
						switch (llvm::cryptoutils->get_range(3)) { // can be improved
							...
							case 1:
								...
								break;
							case 2:
								op = BinaryOperator::Create(Instruction::FSub, i->getOperand(0), i->getOperand(1), *var,
															&*i);
		...
		for (Instruction& I : *alteredBB) {
			if (CallInst* CI = dyn_cast<CallInst>(&I)) {
				if (CI->getCalledFunction() != nullptr && CI->getCalledFunction()->getName().startswith("llvm.dbg")) {
	...
	bool doF(Module& M) {
		...
		for (std::vector<Instruction*>::iterator i = toEdit.begin(); i != toEdit.end(); ++i) {
			...
			Function* emuFunction =
				Function::Create(FunctionType::get(I32Ty, false), GlobalValue::LinkageTypes::PrivateLinkage,
			...
			if (emulateResult == 1) {
				...
				BranchInst::Create(((BranchInst*)*i)->getSuccessor(0), ((BranchInst*)*i)->getSuccessor(1), (Value*)Last,
								   ((BranchInst*)*i)->getParent());
			} else {
				// False, swap operands

				BranchInst::Create(((BranchInst*)*i)->getSuccessor(1), ((BranchInst*)*i)->getSuccessor(0), (Value*)Last,
								   ((BranchInst*)*i)->getParent());
			}
