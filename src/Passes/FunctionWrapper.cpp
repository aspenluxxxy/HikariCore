...
using namespace std;
static cl::opt<int> ProbRate("fw_prob",
...
static cl::opt<int> ObfTimes("fw_times", cl::desc("Choose how many time the FunctionWrapper pass loop on a CallSite"),
...
struct FunctionWrapper : public ModulePass {
	...
	FunctionWrapper(bool flag) : ModulePass(ID) {
		this->flag = flag;
	}
	...
	CallBase* HandleCallSite(CallBase* CS) {
		...
		if (calledFunction == nullptr || (!isa<ConstantExpr>(calledFunction) && !isa<Function>(calledFunction)) ||
		...
		Function* func = Function::Create(ft, GlobalValue::LinkageTypes::InternalLinkage, "HikariFunctionWrapper",
		...
		Value* retval = IRB.CreateCall(FunctionCallee(cast<Function>(calledFunction)), ArrayRef<Value*>(params));
...
INITIALIZE_PASS(FunctionWrapper, "funcwra", "Enable FunctionWrapper.", true, true)
