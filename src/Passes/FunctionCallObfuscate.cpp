...
static cl::opt<uint64_t> dlopen_flag("fco_flag", cl::desc("The value of RTLD_DEFAULT on your platform"),
									 cl::value_desc("value"), cl::init(-1), cl::Optional);
static cl::opt<string> SymbolConfigPath("fcoconfig", cl::desc("FunctionCallObfuscate Configuration Path"),
...
struct FunctionCallObfuscate : public FunctionPass {
	...
	virtual bool runOnFunction(Function& F) override {
		...
		FunctionType* dlopen_type = FunctionType::get(Int8PtrTy, {Int8PtrTy, Int32Ty},
													  false); // int has a length of 32 on both 32/64bit platform
		FunctionType* dlsym_type = FunctionType::get(Int8PtrTy, {Int8PtrTy, Int8PtrTy}, false);
			...
			for (auto I = BB.getFirstInsertionPt(), end = BB.end(); I != end; ++I) {
				...
				if (isa<CallInst>(&Inst) || isa<InvokeInst>(&Inst)) {
					...
					if (calledFunction == NULL) {
						...
						*/
						calledFunction = dyn_cast<Function>(CS->getCalledOperand()->stripPointerCasts());
					}
					// Simple Extracting Failed
					// Use our own implementation
					if (calledFunction == NULL) {
						DEBUG_WITH_TYPE("opt", errs() << "Failed To Extract Function From Indirect Call: "
													  << *CS->getCalledOperand() << "\n");
						continue;
					}
					// It's only safe to restrict our modification to external symbols
					// Otherwise stripped binary will crash
					if (!calledFunction->empty() || calledFunction->getName().equals("dlsym") ||
						calledFunction->getName().equals("dlopen") || calledFunction->isIntrinsic())
					{
						continue;
					}
					// errs()<<"Searching For:"<<calledFunction->getName()<<" In
					// Configuration\n";
					if (this->Configuration.find(calledFunction->getName().str()) != this->Configuration.end()) {
						string sname = this->Configuration[calledFunction->getName().str()].get<string>();
						...
						Value* bitCastedFunction = IRB.CreateBitCast(fp, CS->getCalledOperand()->getType());
