...
bool Flattening::flatten(Function* f) {
	...
	new StoreInst(ConstantInt::get(Type::getInt32Ty(f->getContext()), llvm::cryptoutils->scramble32(0, scrambling_key)),
				  switchVar, insert);
	...
	for (vector<BasicBlock*>::iterator b = origBB.begin(); b != origBB.end(); ++b) {
		...
		numCase = cast<ConstantInt>(ConstantInt::get(
			switchI->getCondition()->getType(), llvm::cryptoutils->scramble32(switchI->getNumCases(), scrambling_key)));
