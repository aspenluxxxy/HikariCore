...
struct StringEncryption : public ModulePass {
	...
	bool runOnModule(Module& M) override {
		...
		for (Module::iterator iter = M.begin(); iter != M.end(); iter++) {
			...
			if (toObfuscate(flag, F, "strenc")) {
				...
				GlobalVariable* GV =
					new GlobalVariable(M, S->getType(), false, GlobalValue::LinkageTypes::PrivateLinkage, S, "");
				encstatus[F] = GV;
				HandleFunction(F);
			}
		}
	...
	void HandleFunction(Function* Func) {
		...
		for (GlobalVariable* GV : Globals) {
			if (GV->hasInitializer() && GV->getSection() != StringRef("llvm.metadata") &&
				...
				GV->getName().find("OBJC") == string::npos)
			{
				if (GV->getInitializer()->getType() ==
					...
					rawStrings.insert(cast<GlobalVariable>(
						cast<ConstantStruct>(GV->getInitializer())->getOperand(2)->stripPointerCasts()));
		...
		for (GlobalVariable* GV : rawStrings) {
			...
			if (intType == Type::getInt8Ty(GV->getParent()->getContext())) {
				...
				DummyConst = ConstantDataArray::get(GV->getParent()->getContext(), ArrayRef<uint8_t>(dummy));
				...
				DummyConst = ConstantDataArray::get(GV->getParent()->getContext(), ArrayRef<uint16_t>(dummy));
				...
				DummyConst = ConstantDataArray::get(GV->getParent()->getContext(), ArrayRef<uint32_t>(dummy));
				...
				DummyConst = ConstantDataArray::get(GV->getParent()->getContext(), ArrayRef<uint64_t>(dummy));
			} else {
			...
			GlobalVariable* EncryptedRawGV = new GlobalVariable(
				*(GV->getParent()), EncryptedConst->getType(), false, GV->getLinkage(), EncryptedConst,
				"EncryptedString", nullptr, GV->getThreadLocalMode(), GV->getType()->getAddressSpace());
			GlobalVariable* DecryptSpaceGV =
				new GlobalVariable(*(GV->getParent()), DummyConst->getType(), false, GV->getLinkage(), DummyConst,
								   "DecryptSpace", nullptr, GV->getThreadLocalMode(), GV->getType()->getAddressSpace());
		...
		for (GlobalVariable* GV : objCStrings) {
			...
			GlobalVariable* EncryptedOCGV =
				ObjectivCString(GV, "EncryptedStringObjC", oldrawString, old2new[oldrawString].first, CS);
			GlobalVariable* DecryptSpaceOCGV =
				ObjectivCString(GV, "DecryptSpaceObjC", oldrawString, old2new[oldrawString].second, CS);
		...
		// Replace Uses
		for (User* U : Users) {
			for (map<GlobalVariable*, pair<GlobalVariable*, GlobalVariable*>>::iterator iter = old2new.begin();
	...
	GlobalVariable* ObjectivCString(GlobalVariable* GV, string name, GlobalVariable* oldrawString,
		...
		return new GlobalVariable(*(GV->getParent()), newCS->getType(), false, GV->getLinkage(), newCS, name.c_str(),
	...
	void HandleDecryptionBlock(BasicBlock* B, BasicBlock* C,
			 ...
			 iter != GV2Keys.end(); ++iter)
		{
			...
			appendToCompilerUsed(*iter->second.second->getParent(), {iter->second.second});
