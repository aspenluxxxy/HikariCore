...
struct AntiClassDump : public ModulePass {
	...
	StringRef getPassName() const override {
		return StringRef("AntiClassDump");
	}
	virtual bool doInitialization(Module& M) override {
		...
		classReplaceMethodTypeArgs.push_back(Int8PtrTy);
		FunctionType* class_replaceMethod_type = FunctionType::get(IMPPointerType, classReplaceMethodTypeArgs, false);
		...
		StructType* objc_property_attribute_t_type =
			reinterpret_cast<StructType*>(M.getTypeByName("struct.objc_property_attribute_t"));
		if (objc_property_attribute_t_type == NULL) {
			...
			M.getOrInsertGlobal("struct.objc_property_attribute_t", objc_property_attribute_t_type);
		}
		...
		FunctionType* addIvarType = FunctionType::get(Int8Ty, ArrayRef<Type*>(addIvarTypeVector), false);
		M.getOrInsertFunction("class_addIvar", addIvarType);
		FunctionType* addPropType = FunctionType::get(Int8Ty, ArrayRef<Type*>(addPropTypeVector), false);
	...
	bool runOnModule(Module& M) override {
		...
		ConstantArray* OBJC_LABEL_CLASS_CDS = dyn_cast<ConstantArray>(OLCGV->getInitializer());

		assert(OBJC_LABEL_CLASS_CDS && "OBJC_LABEL_CLASS_$ Not ConstantArray.Is the target using "
		...
		map<string /*Class*/, GlobalVariable*> GVMapping; // Map ClassName to corresponding GV
		for (unsigned i = 0; i < OBJC_LABEL_CLASS_CDS->getNumOperands(); i++) {
			ConstantExpr* clsEXPR = dyn_cast<ConstantExpr>(OBJC_LABEL_CLASS_CDS->getOperand(i));
		  ...
		  Fifth Operand ClassRO
			*/
			GlobalVariable* SuperClassGV =
				(clsCS->getOperand(1) == NULL) ? NULL : dyn_cast<GlobalVariable>(clsCS->getOperand(1));
			string supclsName = "";
			...
			if (SuperClassGV != NULL) { // We need to handle Classed that doesn't have a base
				...
				supclsName.replace(supclsName.find("OBJC_CLASS_$_"), strlen("OBJC_CLASS_$_"), "");
			}
			dependency[clsName] = supclsName;
			GVMapping[clsName] = CEGV;
			if (supclsName == "" /*NULL Super Class*/ ||
				(SuperClassGV != NULL && !SuperClassGV->hasInitializer() /*External Super Class*/))
			{
		...
		while (tmpclses.size() > 0) {
			...
			if (SuperClassName != "" &&
				std::find(readyclses.begin(), readyclses.end(), SuperClassName) == readyclses.end()) {
	...
	map<string, Value*> splitclass_ro_t(ConstantStruct* class_ro, Module* M) { // Split a class_ro_t structure
		...
		for (unsigned i = 0; i < class_ro->getType()->getNumElements(); i++) {
			...
			if (type == ivar_list_t_type->getPointerTo()) {
				...
				GlobalVariable* methodListGV = cast<GlobalVariable>(methodListCE->getOperand(0));
				...
				info["METHODLIST"] = cast<ConstantArray>(methodListStruct->getOperand(2));
			}
		}
		return info;
	} // splitclass_ro_t
	void handleClass(GlobalVariable* GV, Module* M) {
		assert(GV->hasInitializer() && "ObjC Class Structure's Initializer Missing");
		...
		GlobalVariable* metaclass_ro = cast<GlobalVariable>(
			metaclassGV->getInitializer()->getOperand(metaclassGV->getInitializer()->getNumOperands() - 1));
		IRBuilder<>* IRB = NULL;
		...
		if (Info.find("METHODLIST") != Info.end()) {
			...
			for (unsigned i = 0; i < method_list->getNumOperands(); i++) {
				ConstantStruct* methodStruct = cast<ConstantStruct>(method_list->getOperand(i));
		...
		if (EntryBB == NULL) {
			...
			FunctionType* InitializerType =
				FunctionType::get(Type::getVoidTy(M->getContext()), ArrayRef<Type*>(), false);
			Function* Initializer = Function::Create(InitializerType, GlobalValue::LinkageTypes::PrivateLinkage,
		...
		if (!metaclassCS->getAggregateElement(5)->isNullValue()) {
			...
			GlobalVariable* methodListGV =
				cast<GlobalVariable>(metaclassCS->getAggregateElement(5)->getOperand(0)); // is striped MethodListGV
			...
			Constant* newMethodStruct =
				ConstantStruct::get(newType,
									ArrayRef<Constant*>(newStructValue)); // l_OBJC_$_CLASS_METHODS_
			GlobalVariable* newMethodStructGV =
				new GlobalVariable(*M, newType, true, GlobalValue::LinkageTypes::PrivateLinkage, newMethodStruct,
			...
			Constant* bitcastExpr =
				ConstantExpr::getBitCast(newMethodStructGV, M->getTypeByName("struct.__method_list_t")->getPointerTo());
		...
		Constant* BitCastedIMP = cast<Constant>(
			IRB->CreateBitCast(IRB->GetInsertBlock()->getParent(), objc_getClass->getFunctionType()->getParamType(0)));
		...
		Constant* newMethod = ConstantStruct::get(cast<StructType>(objc_method_type),
		...
		newStructValue.push_back(newMethodList);
		StructType* newType = StructType::get(M->getContext(), ArrayRef<Type*>(newStructType));
		Constant* newMethodStruct = ConstantStruct::get(newType,
														ArrayRef<Constant*>(newStructValue)); // l_OBJC_$_CLASS_METHODS_
		GlobalVariable* newMethodStructGV = new GlobalVariable(
			*M, newType, true, GlobalValue::LinkageTypes::PrivateLinkage, newMethodStruct, "ACDNewInstanceMethodMap");
		appendToCompilerUsed(*M, {newMethodStructGV});
		if (methodListGV) {
			newMethodStructGV->copyAttributesFrom(methodListGV);
		}
	...
	void HandlePropertyIvar(ConstantStruct* class_ro, IRBuilder<>* IRB, Module* M, Value* Class) {
		StructType* objc_property_attribute_t_type =
			reinterpret_cast<StructType*>(M->getTypeByName("struct.objc_property_attribute_t"));
		...
		if (ivar_list != NULL) {
			GlobalVariable* GV =
				dyn_cast<GlobalVariable>(ivar_list->getOperand(0)); // Equal to casted stripPointerCasts()n
			...
			for (unsigned i = 0; i < ivarArray->getNumOperands(); i++) {
				...
				Constant* alignment =
					ConstantInt::get(class_addIvar->getFunctionType()->getParamType(3), alignment_junk);
		...
		if (property_list != NULL) {
			...
			for (unsigned i = 0; i < propArray->getNumOperands(); i++) {
				...
				// GEP's type check
				Value* BitCastedFromArrayToPtr =
					IRB->CreateBitCast(attrInMem, objc_property_attribute_t_type->getPointerTo());
				Value* GEP = IRB->CreateInBoundsGEP(BitCastedFromArrayToPtr, zeroes);
				// Now GEP is done.
				// BitCast it back to our required type
				Value* GEPBitCasedToArg =
					IRB->CreateBitCast(GEP, class_addProperty->getFunctionType()->getParamType(2));
				...
				addProp_args.push_back(
					ConstantInt::get(class_addProperty->getFunctionType()->getParamType(3), attrs.size()));
...
ModulePass* llvm::createAntiClassDumpPass() {
