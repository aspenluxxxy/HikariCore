...
struct IndirectBranch : public FunctionPass {
	...
	bool initialize(Module& M) {
		...
		GlobalVariable* Table = new GlobalVariable(M, AT, false, GlobalValue::LinkageTypes::InternalLinkage,
	...
	bool runOnFunction(Function& Func) override {
		...
		for (BranchInst* BI : BIs) {
			...
			if (BI->isConditional() || indexmap.find(BI->getSuccessor(0)) == indexmap.end()) {
				...
				LoadFrom = new GlobalVariable(*Func.getParent(), AT, false, GlobalValue::LinkageTypes::PrivateLinkage,
			...
			if (BI->isConditional()) {
				...
				index =
					ConstantInt::get(Type::getInt32Ty(Func.getParent()->getContext()), indexmap[BI->getSuccessor(0)]);
			}
...
FunctionPass* llvm::createIndirectBranchPass() {
